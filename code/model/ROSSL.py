...
def build_ROSSL(cfg, z_dim,num_features_nonzero_list, device='cuda'):
    ...
    # Build model
    model =ROSSL(num_features_nonzero_list=num_features_nonzero_list,cfg=cfg, input_dim=input_dim, x_dim=x_dim, num_class=num_class, z_dim=z_dim, activation=activation,
                  dense_x_h=dense_x_h,
...
class ROSSL(nn.Module):
    ...
    def build(self):
        ...
        #     gcn=GCN(self.input_dim, self.x_dim, num_features_nonzero=self.num_features_nonzero_list[i]).to(device=self.device)
        #     # gcn = GCN(self.input_dim, self.x_dim, num_features_nonzero=False).to(
        #     #     device=self.device)
        #     self.gcn.append(gcn)
        #######################
        #### Deterministic ####
